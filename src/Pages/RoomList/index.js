					...
					<div className="content container-fluid">
						<div className="page-header">
							<div className="row align-items-center">
								<div className="col">
									<div className="mt-5">
										<h4 className="card-title float-left mt-2">Customers</h4> <Link to={"/roomlist/add"} className="btn btn-primary float-right veiwbutton">Add Room Category</Link> </div>
								</div>
							</div>
						</div>
						<div className="row">
												...
												<tbody>
													{data && data.map((d, key) =>
														<tr key={d.id}>
															<td >{d.id}</td>
															<td >{d.room_number}</td>
															<td>{d.room_category?.category_name}</td>
															<td>{d.description}</td>
															<td>{d.status}</td>
															<td>
