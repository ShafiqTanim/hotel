...
function MenuItem(){
    ...
    function getDatas() {
        axios.get(`${process.env.REACT_APP_API_URL}/menuitem/index`).then(function(response) {
            setData(response.data.data);
        });
    }
    const deleteData = (id) => {
        axios.delete(`${process.env.REACT_APP_API_URL}/menuitem/${id}`).then(function(response){
            getDatas();
        });
    }
    return(
					...
					<div className="content container-fluid">
						<div className="page-header">
							<div className="row align-items-center">
								<div className="col">
									<div className="mt-5">
										<h4 className="card-title float-left mt-2">Menu Item List</h4> <Link to={"/menuitem/add"} className="btn btn-primary float-right veiwbutton">Add Menu Item List</Link> </div>
								</div>
							</div>
						</div>
						<div className="row">
												...
												<tbody>
													{data && data.map((d, key) =>
														<tr key={d.id}>
															...
															<td>{d.price}</td>
															<td>
																<Link to={`/menuitem/${d.id}`} className='btn btn-info' >Edit</Link>
